...
public:
  ...
  vector<myeosdaemon_def> instances;
...
public:
  ...
  myeosdaemon_def*       instance;
  string          gelf_endpoint;
};

void
myeosdaemon_def::mk_dot_label () {
  dot_label_str = name + "\\nprod=";
...
void
myeosdaemon_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (myeosdaemon_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (myeosdaemon_def &node, host_def *host);
  void launch (myeosdaemon_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, myeosdaemon_def> find_node(uint16_t node_num);
  vector<pair<host_def, myeosdaemon_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("myeosnode", bpo::value<string>(&myeosdaemon_extra_args), "forward myeosnode command line argument(s) to each instance of myeosnode, enclose arg in quotes")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &myeosdaemon : binding.instances) {
          myeosdaemon.host = binding.host_name;
          myeosdaemon.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(myeosdaemon.p2p_port);

          aliases.push_back (myeosdaemon.name);
        }
      }
  ...
  char* erd_env_var = getenv ("MYEOSIO_HOME");
...
void
launcher_def::assign_name (myeosdaemon_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    ...
    for (size_t i = 0; i < (total_nodes); i++) {
      myeosdaemon_def myeosdaemon;
      assign_name(myeosdaemon, i == 0);
      aliases.push_back(myeosdaemon.name);
      myeosdaemon.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(myeosdaemon));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      myeosdaemon_def myeosdaemon;
      assign_name(myeosdaemon, do_bios);
      myeosdaemon.has_db = false;

      if (servers.db.size()) {
        for (auto &dbn : servers.db) {
          if (lhost->host_name == dbn) {
            myeosdaemon.has_db = true;
            break;
         }
        }
      }
      aliases.push_back(myeosdaemon.name);
      myeosdaemon.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(myeosdaemon));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  myeosdaemon_def &instance = *node.instance;
...
void
launcher_def::prep_remote_config_dir (myeosdaemon_def &node, host_def *host) {
...
void
launcher_def::launch (myeosdaemon_def &instance, string &gts) {
  ...
  string myeosdaemoncmd = "programs/myeosnode/myeosnode ";
  if (skip_transaction_signatures) {
    myeosdaemoncmd += "--skip-transaction-signatures ";
  }
  if (!myeosdaemon_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       string args = std::regex_replace (myeosdaemon_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       myeosdaemoncmd += args + " ";
    }
    else {
       myeosdaemoncmd += myeosdaemon_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    myeosdaemoncmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  myeosdaemoncmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  myeosdaemoncmd += " --genesis-json " + genesis.string();
  if (gts.length()) {
    myeosdaemoncmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->myeosio_home + "; nohup " + myeosdaemoncmd + " > "
    ...
    cerr << "spawning child, " << myeosdaemoncmd << endl;

    bp::child c(myeosdaemoncmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << myeosdaemoncmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(myeosdaemon_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, myeosdaemon_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, myeosdaemon_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const myeosdaemon_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.myeosio_home + "; "
                 + "export MYEOSIO_HOME=" + host.myeosio_home + string("; ")
                 + "export MYEOSIO_TN_NODE=" + node_num + "; "
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.myeosio_home + "; "
                 + "export MYEOSIO_HOME=" + host.myeosio_home + "; "
                 + "export MYEOSIO_TN_NODE=" + node_num + "; "
         + "export MYEOSIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
