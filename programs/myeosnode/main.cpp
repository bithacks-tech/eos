...
{
   try {
      app().set_version(myeosio::myeosnode::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "myeosio/myeosnode/data" );
      app().set_default_config_dir(root / "myeosio/myeosnode/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("myeosnode version ${ver}", ("ver", myeosio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("myeosio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
