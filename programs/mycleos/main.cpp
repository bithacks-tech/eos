 ...
 *  @defgroup mycleosenttool MyEOSIO Command Line Interface Client Reference
 *  @brief Tool for sending transactions and querying state from @ref myeosnode
 *  @ingroup mycleosenttool
 */

/**
  @defgroup mycleosenttool

  @section intro Introduction to mycleos

  `mycleos` is a command line tool that interfaces with the REST api exposed by @ref myeosnode. In order to use `mycleos` you will need to
  have a local copy of `myeosnode` running and configured to load the 'myeosio::chain_api_plugin'.

   mycleos contains documentation for all of its commands. For a list of all commands known to mycleos, simply run it with no arguments:
```
$ ./mycleos
Command Line Interface to MyEOSIO Client
Usage: programs/mycleos/mycleos [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where myeosnode is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where mykeosd is running
...
Subcommands:
  ...
  transfer                    Transfer MES from account to account
...
Usage: ./mycleos create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./mycleos create account
Create a new account on the blockchain
Usage: ./mycleos create account [OPTIONS] creator name OwnerKey ActiveKey
...
using namespace myeosio::client::help;
using namespace myeosio::client::http;
using namespace myeosio::client::localize;
using namespace myeosio::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define MESC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
myeosio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      myeosio::client::http::connection_param *cp = new myeosio::client::http::connection_param(context, parse_url(url) + path,
              no_verify ? false : true, headers);

      return myeosio::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to myeosnode at ${u}; is myeosnode running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to mykeosd at ${u}; is mykeosd running?", ("u", url)) << std::endl;
...
myeosio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<myeosio::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   } MES_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      myeosio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = myeosio::chain::authority{1, {{owner, 1}}, {}},
         .active       = myeosio::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } MES_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "MES") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } MES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      MES_ASSERT( myeosio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<myeosio::chain::symbol_code, myeosio::chain::symbol> cache;
   auto a = asset::from_string( s );
   myeosio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<myeosio::chain_apis::read_only::get_currency_stats_result>();
...
void try_local_port( const string& lo_address, uint16_t port, uint32_t duration ) {
   ...
   while ( !local_port_used(lo_address, port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to mykeosd, if mykeosd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to mykeosd")});
      }
   }
}

void ensure_mykeosd_running(CLI::App* app) {
    // get, version, net do not require mykeosd
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by mykeosd
          return;

    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running mycleos like this: ./mycleos ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("mykeosd"); // if mycleos and mykeosd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("mykeosd").append("mykeosd");
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child mykeosd(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (mykeosd.running()) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":" << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start mykeosd because mykeosd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } MES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of MES delegated for net bandwidth")))->required();
      ...
      createAccount->set_callback([this] {
            ...
            } MES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
...
struct vote_producers_subcommand {
   string voter_str;
   vector<myeosio::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<myeosio::chain_apis::read_only::get_table_rows_result>();
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<myeosio::chain_apis::read_only::get_producers_result>();
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of MES to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of MES to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake MES to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of MES to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of MES to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of MES to bid"))->required();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "myeosio")("scope", "myeosio")("table", "namebids")
                               ("lower_bound", myeosio::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<myeosio::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of MES to pay for RAM"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive MES for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<myeosio::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, myeosio::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         if( res.self_delegated_bandwidth.is_object() ) {
            ...
            if( staked.get_symbol() != unstaking.get_symbol() ) {
               // Core symbol of myeosnode responding to the request is different than core symbol built into mycleos
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( now >= refund_time ) {
            std::cout << " (available to claim now with 'myeosio::refund' action)\n";
         } else {
...
int main( int argc, char** argv ) {
   ...
   context = myeosio::client::http::create_http_context();

   CLI::App app{"Command Line Interface to MyEOSIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where myeosnode is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where myeosnode is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where mykeosd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where mykeosd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where myeosnode is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where mykeosd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.set_callback([&app]{ ensure_mykeosd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", myeosio::client::config::version_str)) << std::endl;
   });
   ...
   getAccounts->set_callback([&] {
      ...
      } MES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } MES_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } MES_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } MES_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_abi_callback = [&]() {
      ...
      } MES_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer MES from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending MES"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving MES"))->required();
   transfer->add_option("amount", amount, localized("The amount of MES to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         wallet_key = private_key_type( wallet_key_str );
          ...
          MES_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         MES_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopEnuwallet = wallet->add_subcommand("stop", localized("Stop mykeosd (doesn't work with myeosnode)."), false);
   stopEnuwallet->set_callback([] {
      const auto& v = call(wallet_url, mykeosd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success mykeosd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from myeosnode" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } MES_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } MES_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            MES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", myeosio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            MES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            MES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
