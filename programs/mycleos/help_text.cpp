...
const char* error_advice_public_key_type_exception = R"=====(Public key should be encoded in base58 and starts with MES prefix)=====";
...
const char* error_advice_authority_type_exception = R"=====(Ensure that your authority JSON is valid follows the following format!
{
  ...
  "keys": [         <keys must be alpha-numerically sorted by their string representations and unique>
    ...
    {
      "key":        <STRING: MyEOSIO compatible Public Key>,
...
You can refer to contracts/myeoslib/transaction.hpp for reference)=====";
...
const char* error_advice_tx_duplicate =  "You can try embedding myeosio nonce action inside your transaction to ensure uniqueness.";

const char* error_advice_invalid_action_args_exception = R"=====(Ensure that your arguments follow the contract abi!
You can check the contract's abi by using 'mycleos get code' command.)=====";
...
If you are currently using 'mycleos push action' command, try to add the relevant authority using -p option.)=====";
const char* error_advice_irrelevant_auth_exception =  "Please remove the unnecessary authority from your action!";

const char* error_advice_missing_chain_api_plugin_exception =  "Ensure that you have \033[2mmyeosio::chain_api_plugin\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_wallet_api_plugin_exception =  "Ensure that you have \033[2mmyeosio::wallet_api_plugin\033[0m\033[32m added to your node's configuration!\n"\
                                    "Otherwise specify your wallet location with \033[2m--wallet-url\033[0m\033[32m argument!";
const char* error_advice_missing_history_api_plugin_exception =  "Ensure that you have \033[2mmyeosio::history_api_plugin\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_net_api_plugin_exception =  "Ensure that you have \033[2mmyeosio::net_api_plugin\033[0m\033[32m added to your node's configuration!";
