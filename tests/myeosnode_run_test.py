...
cluster=Cluster(mykeosdd=True, enableMongo=enableMongo, defproduceraPrvtKey=defproduceraPrvtKey, defproducerbPrvtKey=defproducerbPrvtKey)
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.myeosioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    # create accounts via myeosio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.myeosioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.myeosioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.myeosioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.myeosioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.myeosioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.myeosioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    if not enableMongo:
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get MES account failed", raw=True)
