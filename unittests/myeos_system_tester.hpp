...
namespace myeos_system {

class myeos_system_tester : public TESTER {
public:

   myeos_system_tester()
   : myeos_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   myeos_system_tester(Lambda setup) {
      setup(*this);
...
inline uint64_t M( const string& myeos_str ) {
   return core_from_string( myeos_str ).get_amount();
}
