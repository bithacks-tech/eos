...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(myeos.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(myeos.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
      :TESTER(),abi_ser(json::from_string(myeos_token_abi).as<abi_def>())
      {
         create_account( N(myeos.token));
         set_code( N(myeos.token), myeos_token_wast );

         auto result = push_action(N(myeos.token), N(create), mutable_variant_object()
                 ("issuer",       myeos_token)
         ...
         result = push_action(N(myeos.token), N(issue), mutable_variant_object()
                 ("to",       myeos_token)
      ...
      static const std::string myeos_token;
};

const std::string currency_tester::myeos_token = name(N(myeos.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(myeos.token), expected.get_symbol(), N(myeos.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(myeos.token), N(transfer), mutable_variant_object()
         ("from", myeos_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(myeos.token), N(transfer), mutable_variant_object()
                                    ("from", myeos_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(myeos_token, N(alice), "100.0000 CUR");
