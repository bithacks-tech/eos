...
public:
   ...
   myeos_msig_tester() {
      create_accounts( { N(myeos.msig), N(myeos.stake), N(myeos.ram), N(myeos.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, myeos_msig_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("myeosio") + get_balance("myeos.ramfee") + get_balance("myeos.stake") + get_balance("myeos.ram") );
