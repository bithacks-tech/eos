...
shared_ptr<header_state> fork_database::push_block_header( const signed_block_header& b ) { try {
   ...
   if( previous == by_id_idx.end() ) {
      ...
      MES_ASSERT( previous != by_id_idx.end(), unlinkable_block_exception, "block does not link to known chain");
   }

   MES_ASSERT( !(*previous)->invalid, unlinkable_block_exception, "unable to link to a known invalid block" );
   ...
   size_t offset = MES_PERCENT(ibb.size(), config::percent_100- config::irreversible_threshold_percent);
...
void  fork_database::_push_block(const item_ptr& item )
{
   ...
   if( _head && item->previous_id() != block_id_type() )
   {
      ...
      MES_ASSERT(itr != index.end(), unlinkable_block_exception, "block does not link to known chain");
