...
#include <myeosio/chain/webassembly/runtime_interface.hpp>
...
using namespace myeosio::chain::webassembly::common;
...
struct interpreter_interface : ModuleInstance::ExternalInterface {
   ...
   Literal callImport(Import *import, LiteralList &args) override
   {
      ...
      MES_ASSERT(fn_iter != import_lut.end(), wasm_execution_error, "unknown import ${m}:${n}", ("m", import->module.c_str())("n", import->module.c_str()));
   ...
   {
      MES_ASSERT(index < table.size(), wasm_execution_error, "callIndirect: bad pointer");
      auto* func = instance.wasm.getFunctionOrNull(table[index]);
      MES_ASSERT(func, wasm_execution_error, "callIndirect: uninitialized element");
      MES_ASSERT(func->params.size() == arguments.size(), wasm_execution_error, "callIndirect: bad # of arguments");

      for (size_t i = 0; i < func->params.size(); i++) {
         MES_ASSERT(func->params[i] == arguments[i].type, wasm_execution_error, "callIndirect: bad argument type");
      }
      MES_ASSERT(func->result == result, wasm_execution_error, "callIndirect: bad result type");
   ...
   void assert_memory_is_accessible(uint32_t offset, uint32_t size) {
      MES_ASSERT(offset + size <= current_memory_size && offset + size >= offset,
...
class binaryen_runtime : public myeosio::chain::wasm_runtime_interface {
   public:
      binaryen_runtime();
...
inline auto convert_native_to_literal(const interpreter_interface* interface, char* ptr) {
   ...
   MES_ASSERT(ptr >= base && ptr < top_of_memory, wasm_execution_error, "returning pointer not in linear memory");
...
#define _REGISTER_BINARYEN_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static myeosio::chain::webassembly::binaryen::intrinsic_registrator _INTRINSIC_NAME(__binaryen_intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      myeosio::chain::webassembly::binaryen::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\


} } } }// myeosio::chain::webassembly::wavm
