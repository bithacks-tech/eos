...
namespace impl {
   ...
   struct abi_from_variant {
                           ...
                           size_t recursion_depth, const fc::time_point& deadline )
      {
         ...
         FC_ASSERT( fc::time_point::now() < deadline, "serialization time limit ${t}us exceeded", ("t", abi_serializer::max_serialization_time) );
         const variant_object& vo = v.get_object();
         MES_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         MES_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         MES_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
                           ...
                           size_t recursion_depth, const fc::time_point& deadline )
      {
         ...
         FC_ASSERT( fc::time_point::now() < deadline, "serialization time limit ${t}us exceeded", ("t", abi_serializer::max_serialization_time) );
         const variant_object& vo = v.get_object();
         MES_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         MES_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            MES_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
