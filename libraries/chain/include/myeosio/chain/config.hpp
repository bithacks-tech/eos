...
namespace myeosio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(myeosio);
const static uint64_t null_account_name      = N(myeosio.null);
const static uint64_t producers_account_name = N(myeosio.prods);
...
const static uint64_t myeosio_auth_scope       = N(myeosio.auth);
const static uint64_t myeosio_all_scope        = N(myeosio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t myeosio_any_name = N(myeosio.any);
const static uint64_t myeosio_code_name = N(myeosio.code);
...
const static myeosio::chain::wasm_interface::vm_type default_wasm_runtime = myeosio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace myeosio::chain::config

constexpr uint64_t MES_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / myeosio::chain::config::percent_100;
}

template<typename Number>
Number MES_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + myeosio::chain::config::percent_100 - myeosio::chain::config::percent_1)  / myeosio::chain::config::percent_100;
}
