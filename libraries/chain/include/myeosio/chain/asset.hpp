...
struct asset
{
   ...
   explicit asset(share_type a = 0, symbol id = symbol(CORE_SYMBOL)) :amount(a), sym(id) {
      MES_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
      MES_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
   }
...
namespace fc {
inline void to_variant(const myeosio::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, myeosio::chain::asset& vo) {
   vo = myeosio::chain::asset::from_string(var.get_string());
}
}

FC_REFLECT(myeosio::chain::asset, (amount)(sym))
FC_REFLECT(myeosio::chain::extended_asset, (quantity)(contract) )
