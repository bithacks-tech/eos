...
#define N(X) myeosio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<myeosio::chain::name> : private hash<uint64_t> {
      typedef myeosio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const myeosio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, myeosio::chain::name& check);
} // fc


FC_REFLECT( myeosio::chain::name, (value) )
