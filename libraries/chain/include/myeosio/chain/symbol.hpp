...
#include <myeosio/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
namespace myeosio {
   namespace chain {
      ...
      /**
         ...
         from_string constructs a symbol from an input a string of the form "4,MES"
...
#define SY(P,X) ::myeosio::chain::string_to_symbol_c(P,#X)
      ...
      class symbol {
         public:
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = myeosio::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
...
namespace fc {
   inline void to_variant(const myeosio::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, myeosio::chain::symbol& vo) {
      vo = myeosio::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const myeosio::chain::symbol_code& var, fc::variant& vo) {
      vo = myeosio::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, myeosio::chain::symbol_code& vo) {
      vo = myeosio::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(myeosio::chain::symbol_code, (value))
FC_REFLECT(myeosio::chain::symbol, (m_value))
FC_REFLECT(myeosio::chain::extended_symbol, (sym)(contract))
