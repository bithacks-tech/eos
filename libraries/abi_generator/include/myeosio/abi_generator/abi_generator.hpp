...
namespace myeosio {
      ...
      FC_EXPAND_MACRO( \
       FC_MULTILINE_MACRO_BEGIN \
         ...
         {                                                                      \
           ...
           FC_THROW_EXCEPTION( myeosio::abi_generation_exception, #TEST ": "  __VA_ARGS__ ); \
         }                                                                      \
   ...
   /**
     * @brief Generates myeosio::abi_def struct handling events from ASTConsumer
     */
   class abi_generator {
      private:
   ...
   struct find_myeosio_abi_macro_action : public PreprocessOnlyAction {

         string& contract;
         vector<string>& actions;
         const string& abi_context;

         find_myeosio_abi_macro_action(string& contract, vector<string>& actions, const string& abi_context
            ): contract(contract),
         ...
         struct callback_handler : public PPCallbacks {
            ...
            CompilerInstance& compiler_instance;
            find_myeosio_abi_macro_action& act;

            callback_handler(CompilerInstance& compiler_instance, find_myeosio_abi_macro_action& act)
            ...
            void MacroExpands (const Token &token, const MacroDefinition &md, SourceRange range, const MacroArgs *args) override {
               ...
               if( id->getName() != "MYEOSIO_ABI" ) return;
