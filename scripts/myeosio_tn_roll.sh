#!/bin/bash
#
# myeosio-tn_roll is used to have all of the instances of the MyEOSIO daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: myeosio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./myeosio-tn_roll.sh is sufficient.
#

if [ -z "$MYEOSIO_HOME" ]; then
    echo MYEOSIO_HOME not set - $0 unable to proceed.
    exit -1
fi
...
bash $MYEOSIO_HOME/scripts/myeosio-tn_down.sh

cp $SDIR/$RD/$prog $RD/$prog

if [ $DD = "all" ]; then
    for MYEOSIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $MYEOSIO_HOME/scripts/myeosio-tn_up.sh $*
    done
else
    bash $MYEOSIO_HOME/scripts/myeosio-tn_up.sh $*
fi
unset MYEOSIO_RESTART_DATA_DIR
