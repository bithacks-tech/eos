...
The next term is "node." A node is an instance of the myeosdaemon executable configured to serve as 0 or more producers. There is not a one-to-one mapping between nodes and hosts, a host may serve more than one node, but one node cannot span more than one host. 
...
Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping myeosdaemon instances, but with whom you may wish to collaborate for setting up a decentralized testnet.
...
Note that the Launcher will not push instances of myeosdaemon to the remote hosts, you must prepare the various test network hosts separately.
...
To address the complexity implied by distributing multiple myeosdaemon nodes across a LAN or a wider network, the launcher application was created. 

Based on a handful of command line arguments the Launcher is able to compose per-node configuration files, distribute these files securely amongst the peer hosts, then start up the multiple instances of myeosdaemon.

Instances of myeosdaemon started this way have their output logged in individual text files. Finally the launcher application is also able to shut down some or all of the test network. 

## Running the Launcher application

The launcher program is used to configure and deploy producing and non-producing myeosdaemon nodes that talk to each other using configured routes. The configuration for each node is stored in separate directories, permitting multiple nodes to be active on the same host, assuming the machine has sufficient memory and disk space for multiple myeosdaemon instances. The launcher makes use of multiple configuration sources in order to deploy a testnet. A handful of command line arguments can be used to set up simple local networks. 
...
launcher command line arguments:
  ...
  -d [ --delay ] arg (=0)               number of seconds to wait before starting the next node. Used to simulate a person keying in a series of individual myeosdaemon startup command lines.
...
In this mode, the launcher does not activate any myeosdaemon instances, it produces a file of the given filename. This file is a JSON formatted template that provides an easy means of 
...
```
  "nodes": [[
      "testnet_0",{
        ...
        "ssh_identity": "",
        "ssh_args": "",
        "myeos_root_dir": "/home/phil/blockchain/myeosio",
        "data_dir": "tn_data_0",
        "hostname": "remoteserv",
...
| nodes         | a collection of descriptors defining the myeosdaemon instances used to assemble this testnet. The names used as keys in this collection are also aliases used within as placeholders for peer nodes.
...
| myeos_root_dir       | specifies the directory into which all myeosdaemon artifacts are based. This is required for any hosts that are not the local host.
...
It is also necessary to provision the server by at least copying the myeosdaemon executable, and the genesis.json files to their appropriate locations relative to some named MyEOSIO root directory. For example, I defined the MyEOSIO root to be `/home/phil/blockchain/myeos`. When run, the launcher will run through a variety of shell commands using ssh and finally using scp to copy a config.ini file to the appropriate data directory on the remote.
...
| :------------ | :----------
| config.ini    | The myeosdaemon configuration file.
| myeosdaemon.pid      | The process ID of the running myeosdaemon instance.
| blockchain/*  | The blockchain backing store
| blocks/*      | The blockchain log store
| stderr.txt    | The cerr output from myeosdaemon.
| stdout.txt    | The cout output from myeosdaemon.
