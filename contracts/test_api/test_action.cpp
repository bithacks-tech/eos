...
void test_action::read_action_normal() {
   ...
   myeosio_assert(action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)");

   total = read_action_data(buffer, 30);
   myeosio_assert(total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data(buffer, 100);
   myeosio_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   myeosio_assert(total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   myeosio_assert(total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   myeosio_assert(dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A");
   myeosio_assert(dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B");
   myeosio_assert(dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C");
}
...
void test_action::test_dummy_action() {
   ...
   myeosio_assert( total > 0, "get_action failed" );
   myeosio::action act = myeosio::get_action( 1, 0 );
   myeosio_assert( act.authorization.back().actor == N(testapi), "incorrect permission actor" );
   myeosio_assert( act.authorization.back().permission == N(active), "incorrect permission name" );
   myeosio_assert( myeosio::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   myeosio_assert( act.account == N(testapi), "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      myeosio_assert(false, "get_context_free_data() not allowed in non-context free action");
   } else {
      myeosio_assert(dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A");
      myeosio_assert(dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B");
      myeosio_assert(dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C");
   }
}
...
void test_action::test_cf_action() {
   ...
   myeosio::action act = myeosio::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      myeosio_assert( size > 0, "size determination failed" );
      myeosio::bytes cfd( static_cast<size_t>(size) );
      size = get_context_free_data( cfa.cfd_idx, &cfd[0], static_cast<size_t>(size) );
      myeosio_assert(static_cast<size_t>(size) == cfd.size(), "get_context_free_data failed" );
      uint32_t v = myeosio::unpack<uint32_t>( &cfd[0], cfd.size() );
      myeosio_assert( v == cfa.payload, "invalid value" );
      ...
      myeosio::action dum_act;
      dum_act.send();
      myeosio_assert( false, "action send should not be allowed" );
   } else if ( cfa.payload == 206 ) {
      myeosio::require_auth(N(test));
      myeosio_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      myeosio_assert( false, "system_api should not be allowed" );
      ...
      myeosio_assert( false, "transaction_api should not be allowed" );
...
void test_action::require_notice(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( receiver == N(testapi) ) {
      myeosio::require_recipient( N(acc1) );
      myeosio::require_recipient( N(acc2) );
      myeosio::require_recipient( N(acc1), N(acc2) );
      myeosio_assert(false, "Should've failed");
...
void test_action::require_auth() {
   prints("require_auth");
   myeosio::require_auth( N(acc3) );
   myeosio::require_auth( N(acc4) );
}

void test_action::assert_false() {
   myeosio_assert(false, "test_action::assert_false");
}

void test_action::assert_true() {
   myeosio_assert(true, "test_action::assert_true");
}
...
void test_action::test_abort() {
   abort();
   myeosio_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint64_t pub_time = 0;
   uint32_t total = read_action_data(&pub_time, sizeof(uint64_t));
   myeosio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   myeosio_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   myeosio_assert( receiver == cur_rec, "the current receiver does not match" );
}
...
void test_action::test_current_time() {
   ...
   myeosio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   myeosio_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_assert_code() {
   ...
   myeosio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   myeosio_assert_code( false, code );
}
